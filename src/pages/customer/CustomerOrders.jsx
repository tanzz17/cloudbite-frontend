...
export function CustomerOrders() {
  ...
  useEffect(() => {
    customerAPI.getOrders()
      .then((response) => setOrders(asArray(response.data)))
  ...
  return (
    <div className="min-h-screen bg-brand-cream dark:bg-brand-dark-bg px-4 py-8 max-w-2xl mx-auto">
      ...
      {loading ? (
        <div className="space-y-4">{[...Array(4)].map((_, index) => <div key={index} className="h-28 rounded-3xl shimmer" />)}</div>
      ) : orders.length === 0 ? (
        <EmptyState
          icon="Orders"
          title="No orders yet"
          message="Place your first order from a cloud kitchen."
        ...
        <div className="space-y-4">
          {orders.map((order) => (
            ...
            >
              <div className="w-12 h-12 rounded-2xl bg-orange-50 dark:bg-orange-900/20 flex items-center justify-center text-sm flex-shrink-0">Meal</div>
...
export function OrderDetail() {
  ...
  const didSyncFromReturnRef = useRef(false)
  ...
  useEffect(() => {
    const returnedFromHostedPayment =
      searchParams.get('payment_return') === 'razorpay' ||
      searchParams.has('razorpay_payment_link_id') ||
      searchParams.has('razorpay_payment_id')

    if (!order || !returnedFromHostedPayment || didSyncFromReturnRef.current) {
      return
    }

    didSyncFromReturnRef.current = true
    setSyncingPayment(true)

    paymentAPI.syncPaymentStatus(order.id)
      .then(async ({ data }) => {
        await fetchOrder()
        if (data?.paymentStatus === 'COMPLETED') {
          ...
          toast('Payment is still pending confirmation.', { icon: '⏳' })
        }
      })
      .catch((error) => {
  ...
  }, [fetchOrder, order, searchParams, stripPaymentParams])
  ...
  useWebSocket(order ? [
    {
      ...
      callback: (message) => {
        setOrder((previous) => ({ ...previous, status: message.status }))
        toast(`Order status: ${message.status.replace(/_/g, ' ')}`, { icon: '🔔' })
      },
    },
    {
      topic: `/topic/order/${orderId}/location`,
      callback: (location) => setPartnerLocation({ lat: location.lat, lng: location.lng }),
    },
  ] : [])
    ...
    return (
      <div className="min-h-screen bg-brand-cream dark:bg-brand-dark-bg flex items-center justify-center">
        <div className="text-center">
          <div className="text-5xl mb-4 animate-bounce-soft">Loading</div>
          <p className="font-hand text-orange-600 text-xl">Loading order...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-brand-cream dark:bg-brand-dark-bg">
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-6">
          <button onClick={() => navigate('/orders')} className="text-2xl" aria-label="Back to orders">←</button>
          <div>
        ...
        <div className="card p-6 mb-4">
          ...
          {order.status !== 'CANCELLED' ? (
                  ...
                  <div key={step.key} className="flex flex-col items-center gap-1.5">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-500 ${
                        index <= currentStep ? 'bg-orange-gradient text-white shadow-orange' : 'bg-gray-100 dark:bg-brand-dark-border text-gray-400'
                      }`}
                    >
                      {index < currentStep ? '✓' : step.icon}
                    </div>
                    <span
                      className={`text-[10px] font-body text-center leading-tight max-w-[60px] ${
                        index <= currentStep ? 'text-orange-600 dark:text-orange-400 font-bold' : 'text-gray-400'
                      }`}
                    >
                      {step.label}
                    </span>
            ...
            <div className="mt-4 p-4 bg-red-50 dark:bg-red-900/20 rounded-2xl text-center">
              <p className="font-body font-bold text-red-600 dark:text-red-400">Order Cancelled</p>
        ...
        {['PARTNER_ASSIGNED', 'HANDOVER', 'OUT_FOR_DELIVERY'].includes(order.status) && (
          <div className="card p-4 mb-4">
            ...
            <div className="h-48 rounded-2xl bg-orange-50 dark:bg-brand-dark-border flex items-center justify-center">
              {partnerLocation ? (
                <div className="text-center">
                  <div className="text-4xl mb-2 animate-bounce-soft">Rider</div>
                ...
                <div className="text-center">
                  <div className="text-4xl mb-2 animate-pulse">Map</div>
          ...
          <div className="space-y-2">
            {order.items?.map((item) => (
              <div key={item.id} className="flex items-center justify-between text-sm font-body">
                <span className="text-gray-600 dark:text-gray-400">{item.quantity}x {item.itemName}</span>
            ...
            {order.paymentMethod === 'RAZORPAY' && order.paymentStatus !== 'COMPLETED' && (
              <button
                ...
                disabled={retryingPayment || syncingPayment}
                className="btn-primary mt-4 flex items-center justify-center gap-2 px-5 py-3 text-sm"
              >
                {retryingPayment || syncingPayment ? (
                  <>
                    <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                    {syncingPayment ? 'Refreshing payment...' : 'Opening secure payment...'}
                  </>
                ) : (
                  'Complete Payment'
                )}
