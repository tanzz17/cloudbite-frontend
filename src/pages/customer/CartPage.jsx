...
export default function CartPage() {
  ...
  const subtotal = items.reduce((sum, i) => sum + ((i.menuItem?.price || 0) * i.quantity), 0)
  ...
  if (items.length === 0) return (
    <div className="min-h-screen bg-brand-cream dark:bg-brand-dark-bg flex items-center justify-center px-4">
      <div className="text-center">
        ...
        <p className="font-body text-gray-500 dark:text-gray-400 mb-6">Add some delicious dishes to get started!</p>
          ...
          <div className="lg:col-span-2 space-y-4">
              ...
              <div key={item.id} className="card p-4 flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <button onClick={() => handleQty(item.id, item.quantity - 1)}
                    className="w-8 h-8 rounded-full border-2 border-orange-400 text-orange-600 flex items-center justify-center font-bold hover:bg-orange-50 transition-colors">−</button>
                  <span className="w-8 text-center font-body font-bold text-gray-900 dark:text-white">{item.quantity}</span>
                  <button onClick={() => handleQty(item.id, item.quantity + 1)}
                    className="w-8 h-8 rounded-full bg-orange-gradient text-white flex items-center justify-center font-bold hover:shadow-orange transition-all">+</button>
                </div>
            ...
            <div className="card p-4">
              ...
              <textarea value={deliveryAddress} onChange={e => setDeliveryAddress(e.target.value)}
                placeholder="Enter your full delivery address..." className="input-field resize-none" rows={3} />
              <input value={instructions} onChange={e => setInstructions(e.target.value)}
                placeholder="Special instructions (optional)..." className="input-field mt-3" />
            </div>
            ...
            <div className="card p-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[{ id: 'COD', label: 'Cash on Delivery', icon: '💵' }, { id: 'RAZORPAY', label: 'Pay Online', icon: '💳' }].map(({ id, label, icon }) => (
                  <button key={id} onClick={() => setPaymentMethod(id)}
                    className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${
                      paymentMethod === id ? 'border-orange-500 bg-orange-50 dark:bg-orange-900/20' : 'border-orange-100 dark:border-brand-dark-border bg-white dark:bg-brand-dark-card'
                    }`}>
            ...
            <div className="card p-5 sticky top-4">
              ...
              <div className="space-y-3 text-sm font-body">
                <div className="flex justify-between text-gray-600 dark:text-gray-400">
                  <span>Subtotal ({items.length} items)</span><span>{formatCurrency(subtotal)}</span>
                </div>
                <div className="flex justify-between text-gray-600 dark:text-gray-400">
                ...
                <div className="border-t border-orange-100 dark:border-brand-dark-border pt-3 flex justify-between font-display font-bold text-base text-gray-900 dark:text-white">
                  <span>Total</span><span className="text-orange-600 dark:text-orange-400">{formatCurrency(total)}</span>
                </div>
              </div>
              <button onClick={handleCheckout} disabled={placing}
                ...
                {placing
                  ...
                  : paymentMethod === 'RAZORPAY' ? '💳 Pay & Order' : '🛒 Place Order'}
              </button>
              <p className="text-center text-xs font-body text-gray-400 mt-3">
                {paymentMethod === 'COD' ? 'Pay cash when delivered' : 'Secure payment via Razorpay'}
              </p>
            </div>
          </div>
        </div>
